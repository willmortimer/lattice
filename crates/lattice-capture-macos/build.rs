//! Build glue for `lattice-capture-macos`.
//!
//! On macOS, locates or builds `libLatticeCaptureBridge.dylib` and emits link
...
fn main() {
    ...
    if target_os != "macos" {
        println!(
            "cargo:warning=lattice-capture-macos is a macOS capture bridge; \
    ...
    if !should_link_bridge() {
        println!(
            "cargo:warning=LatticeCaptureBridge not linked (enable `link-bridge` or \
             `live-capture` feature to link the Swift dylib)"
        );
        return;
    }
    ...
    println!(
        "cargo:warning=LatticeCaptureBridge not linked. Build with: \
         cd crates/lattice-capture-macos/swift && swift build -c release. \
         Or set LATTICE_CAPTURE_BRIDGE_LIB to the directory containing \
         libLatticeCaptureBridge.dylib."
    );
}

fn resolve_lib_dir(default_products: &Path) -> Option<PathBuf> {
    if let Ok(lib_dir) = std::env::var("LATTICE_CAPTURE_BRIDGE_LIB") {
...
fn dylib_exists(dir: &Path) -> bool {
    dir.join("libLatticeCaptureBridge.dylib").is_file()
}
...
fn emit_link(lib_dir: &Path) {
    ...
    let dylib = lib_dir.join("libLatticeCaptureBridge.dylib");

    if let Some(profile_dir) = profile_target_dir() {
        let dest = profile_dir.join("libLatticeCaptureBridge.dylib");
        if let Err(err) = std::fs::copy(&dylib, &dest) {
            println!(
                "cargo:warning=failed to copy LatticeCaptureBridge to {}: {err}",
                dest.display()
            );
        }
        let deps = profile_dir.join("deps").join("libLatticeCaptureBridge.dylib");
        let _ = std::fs::copy(&dylib, deps);
    }

    println!("cargo:rustc-cfg=link_bridge");
    println!("cargo:rustc-link-search=native={}", lib_dir.display());
    println!("cargo:rustc-link-lib=dylib=LatticeCaptureBridge");
    println!("cargo:rustc-link-arg=-Wl,-rpath,{}", lib_dir.display());
    println!(
        "cargo:warning=Linking LatticeCaptureBridge from {}",
        lib_dir.display()
    );
}
