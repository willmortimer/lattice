//! Build glue for `lattice-approval-macos` Secure Enclave Swift bridge.
...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=LATTICE_APPROVAL_BRIDGE_LIB");
    println!("cargo:rerun-if-changed=swift/Package.swift");
    println!("cargo:rerun-if-changed=include/lattice_approval_bridge.h");
    ...
    if !should_link_bridge() {
        println!(
            "cargo:warning=LatticeApprovalBridge not linked (enable `link-bridge` feature)"
        );
        return;
    }
    ...
    println!(
        "cargo:warning=LatticeApprovalBridge not linked. Build with: \
         cd crates/lattice-approval-macos/swift && swift build -c release"
    );
}

fn resolve_lib_dir(default_products: &Path) -> Option<PathBuf> {
    if let Ok(lib_dir) = std::env::var("LATTICE_APPROVAL_BRIDGE_LIB") {
...
fn dylib_exists(dir: &Path) -> bool {
    dir.join("libLatticeApprovalBridge.dylib").is_file()
}
...
fn emit_link(lib_dir: &Path) {
    ...
    let dylib = lib_dir.join("libLatticeApprovalBridge.dylib");

    if let Some(profile_dir) = profile_target_dir() {
        let dest = profile_dir.join("libLatticeApprovalBridge.dylib");
        let _ = std::fs::copy(&dylib, &dest);
        let deps = profile_dir
            .join("deps")
            .join("libLatticeApprovalBridge.dylib");
        let _ = std::fs::copy(&dylib, deps);
    }

    println!("cargo:rustc-cfg=link_bridge");
    println!("cargo:rustc-link-search=native={}", lib_dir.display());
    println!("cargo:rustc-link-lib=dylib=LatticeApprovalBridge");
    println!("cargo:rustc-link-arg=-Wl,-rpath,{}", lib_dir.display());
    println!(
        "cargo:warning=Linking LatticeApprovalBridge from {}",
        lib_dir.display()
    );
}
