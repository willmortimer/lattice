...
--execution-mode=oci with KernelFS role volume sources from kernelfs export:
  ...
  ${CELL_VZ_RUNTIME_DIR}/ivisor-worker-<cell-id>/agent-share/.kernelfs-runs/{run_id}/{input,output[,work]}

Materialize + volume sources are nested per run_id under agent-share/.kernelfs-runs/.
