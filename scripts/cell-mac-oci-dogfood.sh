...
--execution-mode=oci with KernelFS role host dirs under:

  ${CELL_VZ_RUNTIME_DIR}/ivisor-worker-<cell-id>/agent-share/{input,output[,work]}

Contract: Cell docs/mac-live-bind-demo.md (agent-share layout + VirtioFS live-bind).
...
Live options (forwarded; --execution-mode=oci is always set):
  ...
  --with-work               also create/mount agent-share/work
