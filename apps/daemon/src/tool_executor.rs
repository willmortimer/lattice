...
use lattice_mcp_catalog::{
    TOOL_WORKSPACE_BUILD_CONTEXT, TOOL_WORKSPACE_DATASET_GET_SCHEMA,
...
pub fn execute(runtime: &LatticeRuntime, call: ToolCall) -> Result<Value, ToolError> {
    match resolve_tool(&call.name) {
        Some(ToolKind::Search) => call_search(runtime, call.arguments),
        Some(ToolKind::Read) => call_read(runtime, call.arguments),
        Some(ToolKind::Related) => call_related(runtime, call.arguments),
        Some(ToolKind::BuildContext) => call_build_context(runtime, call.arguments),
...
fn call_search(runtime: &LatticeRuntime, args: Value) -> Result<Value, ToolError> {
    let params: SearchParams =
        serde_json::from_value(args).map_err(|e| api_to_tool_error(ApiError::BadRequest(e.to_string())))?;
    let response = api_search(runtime, params).map_err(api_to_tool_error)?;
    serde_json::to_value(response).map_err(|e| api_to_tool_error(ApiError::Internal(e.to_string())))
}

fn call_read(runtime: &LatticeRuntime, args: Value) -> Result<Value, ToolError> {
    let params: ReadParams =
        serde_json::from_value(args).map_err(|e| api_to_tool_error(ApiError::BadRequest(e.to_string())))?;
    let response = api_read(runtime, params).map_err(api_to_tool_error)?;
    serde_json::to_value(response).map_err(|e| api_to_tool_error(ApiError::Internal(e.to_string())))
}

fn call_related(runtime: &LatticeRuntime, args: Value) -> Result<Value, ToolError> {
    let params: RelatedParams =
        serde_json::from_value(args).map_err(|e| api_to_tool_error(ApiError::BadRequest(e.to_string())))?;
    let response = api_related(runtime, params).map_err(api_to_tool_error)?;
    serde_json::to_value(response).map_err(|e| api_to_tool_error(ApiError::Internal(e.to_string())))
}

fn call_build_context(runtime: &LatticeRuntime, args: Value) -> Result<Value, ToolError> {
    let params: BuildContextParams =
        serde_json::from_value(args).map_err(|e| api_to_tool_error(ApiError::BadRequest(e.to_string())))?;
    let response = api_build_context(runtime, params).map_err(api_to_tool_error)?;
    serde_json::to_value(response).map_err(|e| api_to_tool_error(ApiError::Internal(e.to_string())))
}
...
mod tests {
    ...
    fn execute_search_round_trip() {
        ...
        std::fs::write(dir.path().join("Page.md"), "# Hello searchable-executor-token\n").unwrap();
    ...
    fn short_name_alias_search() {
        ...
        let text = value.to_string();
        assert!(
            !value["hits"].as_array().map(|h| h.is_empty()).unwrap_or(true),
