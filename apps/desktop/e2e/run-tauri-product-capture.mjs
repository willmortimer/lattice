#!/usr/bin/env node
/**
 * Starts Lattice with `e2e-testing` + First Look reset, runs the native
 * Remotion product capture (`e2e/capture/product.capture.tauri.spec.ts`),
 * then tears the app down.
 *
 * Set `LATTICE_PERF_REUSE_TAURI=1` to attach to an already-running
 * `pnpm tauri:dev:e2e` instead of spawning one.
 *
 * Output defaults to the private ecosystem Remotion `public/product` folder
 * (override with `YC_REMOTION_ASSETS`).
 *
 * Requires: macOS Screen Recording permission, ffmpeg on PATH for MP4 clips.
 */
...
const smokeSpec = "e2e/capture/product.capture.tauri.spec.ts";
const defaultAssets = resolve(
  ecosystemRoot,
    ...
    const child = spawn(
      ...
      ["exec", "playwright", "test", "--project=tauri", smokeSpec],
      {
        ...
        env: {
          ...
          YC_REMOTION_ASSETS: process.env.YC_REMOTION_ASSETS ?? defaultAssets,
        },
      },
    );
