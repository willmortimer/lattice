#!/usr/bin/env node
/**
 * Starts Lattice with `e2e-testing` + First Look reset, runs the analytics
 * Tauri smoke (`e2e/data/analytics.smoke.tauri.spec.ts`), then tears the app down.
 *
 * Set `LATTICE_PERF_REUSE_TAURI=1` to attach to an already-running
 * `pnpm tauri:dev:e2e` instead of spawning one.
 *
 * Not a CI gate — local / optional confidence only.
 */
...
const smokeSpec = "e2e/data/analytics.smoke.tauri.spec.ts";
  ...
  if (!reused) {
    ...
    app = spawn("pnpm", ["tauri", "dev", "--features", "e2e-testing"], {
      ...
      env: {
        ...
        // Match `pnpm tauri:dev:e2e` so Orders / Dashboards / canvas match the demo seed.
