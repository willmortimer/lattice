#!/usr/bin/env node
/**
 * Starts Lattice with `e2e-testing` + First Look reset, runs the schema /
 * tabular-import Tauri smoke (`e2e/data/schema.smoke.tauri.spec.ts`), then
 * tears the app down.
 *
...
const smokeSpec = "e2e/data/schema.smoke.tauri.spec.ts";
  ...
  if (!reused) {
    ...
    app = spawn("pnpm", ["tauri", "dev", "--features", "e2e-testing"], {
      ...
      env: {
        ...
        // Match `pnpm tauri:dev:e2e` so CRM + Data/sample.csv match the demo seed.
