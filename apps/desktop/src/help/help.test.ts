...
describe("help search filter", () => {
  it("filters pages by title and body text", () => {
    ...
    const downloadMatches = filterHelpPages(CORPUS.pages, "not on this device");
    expect(downloadMatches.some((page) => page.stem === "what-to-click")).toBe(true);
    expect(downloadMatches.some((page) => page.stem === "first-workspace")).toBe(true);
