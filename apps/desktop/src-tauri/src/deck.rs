...
fn open_deck_package(
    root: &Path,
    rel_path: &str,
) -> Result<(Workspace, PathBuf, PathBuf, String), String> {
    ...
    Ok((
        workspace,
        package.clone(),
        resolve_deck_manifest_path(&package),
        package_path,
    ))
}
...
pub fn deck_load_session(request: DeckLoadRequest) -> Result<DeckSessionView, String> {
    let (_workspace, package, manifest_path, package_path) =
        open_deck_package(Path::new(&request.root), &request.rel_path)?;
    let manifest = DeckManifest::load(&manifest_path).map_err(|error| error.to_string())?;
    let theme_css = match &manifest.theme.stylesheet {
        Some(reference) => bounded_utf8(
            &package.join(reference),
            MAX_THEME_CSS_BYTES,
            "deck theme stylesheet",
        )?,
        None => String::new(),
    };
    ...
    for slide in &manifest.slides {
        slides.push(DeckSlideView {
            ...
            notes: slide
                ...
                .as_ref()
                .map(|notes| {
                    bounded_utf8(&package.join(notes), MAX_NOTES_BYTES, "deck speaker notes")
                })
                .transpose()?,
