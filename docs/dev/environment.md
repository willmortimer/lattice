...
| `LATTICE_DEV_DEMO_OVERLAY` | `nxr desktop-dev` / `scripts/exec-with-ai-env.sh` (auto when nested under `lattice-ecosystem`), or your shell | absolute path to a directory whose layout mirrors workspace-relative paths | After First Look seed/reset (and on subsequent ensure when the workspace already exists), merge files from this directory into the workspace root. Used for private ecosystem overlays such as `demos/hackathon-pitch` (`Hackathon/Pitch.deck`) — never baked into the public demo template. | No | Works today |
