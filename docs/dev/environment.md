...
| `LATTICE_DEV_DEMO_OVERLAY` | ecosystem `scripts/exec-for-dev.sh` (default: `demos/hackathon-pitch`), or your shell | absolute path to a directory whose layout mirrors workspace-relative paths | After First Look seed/reset (and on subsequent ensure when the workspace already exists), merge files from this directory into the workspace root. Used for private ecosystem overlays such as `Hackathon/Pitch.deck` — never baked into the public demo template. Public `nxr desktop-dev` inside `lattice/` alone does not set this; use umbrella `nxr desktop-dev` / `exec-for-dev`. | No | Works today |
| `LATTICE_DEMO_DRIVER` | ecosystem `scripts/exec-for-dev.sh --demo-driver` / `--capture-yc` only | `1`, `true`, or `yes` | Mounts the in-app demo driver (Play scene / ⌘⇧D) that runs the shared scene script via DOM. Intended only when launched through `exec-for-dev` — not for released builds. | No | Works today |
| `LATTICE_DEMO_SCENE` | `exec-for-dev` (default `demos/hackathon-pitch/scene.json` when driver/capture) | absolute path to scene JSON | Shared beat list for capture Playwright, in-app driver, and Remotion montage/chapters/VO. | No | Works today |
| `LATTICE_HOME` | your shell | any writable directory | Override the Lattice profile root (`~/Lattice` in release; in **debug** builds without `LATTICE_DEV_HOME`, the default is `target/dev-home` under the process cwd). Relative values are resolved against the process current directory. Ignored when `LATTICE_DEV_HOME` is set. Selects Personal seeding instead of First Look. On Windows, when `dirs::home_dir()` is unavailable, Lattice falls back to `USERPROFILE` before failing. | No | Works today |
...
| `LATTICE_GITHUB_APP_CLIENT_ID` | `.env` / shell | GitHub App client id | GitHub App connect ([ADR 0045](../decisions/0045-github-connected-repos-are-readonly-extracts.md)): desktop browser OAuth + CLI device flow. App needs Contents + Metadata read-only; callback `http://127.0.0.1:17872/callback`. | No | Works today |
...
| `LATTICE_SKIP_PRESENCE` | e2e / automation shell | any non-empty value | Skip Touch ID / device-owner prompts on privileged applies (`apply_proposal_cmd`, link-repair apply). Also skipped automatically when the desktop is built with `--features e2e-testing`. | No | Works today |
