...
| 8 | ⌘-click multi-select + drag move | **pass** (selection/move UI) / known skip repair | Tree is `aria-multiselectable`; batch move (2+) intentionally skips link-repair orchestration (Wave-1 known). Browser remaps locally; native `moveResources`. Validation covered by `treeOps.test.ts`. | `ResourceTree.tsx:396`; `useResourceController.ts:549–551`, `618–646`; `docs/39-resource-runtime-contracts.md:66–68` |
